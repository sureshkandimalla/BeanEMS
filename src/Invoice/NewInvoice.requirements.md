...
Purpose: "Add New Invoice" form used to create a single invoice record, linking an Employee, Customer, and Project.

Each requirement below is numbered (`R1`, `R2`, ...) so test cases can reference them directly (e.g. `TC-R3-01`).

## 1. Data loading

- **R1.1** On mount, the form fetches Employees (`getEmployees`), Customers (`getAllCustomers`), and Projects (`getProjects`) in parallel and shows a centered spinner until all three resolve.
- **R1.2** If any of the three fetches fail, an error modal ("Error fetching employees or customers. Please try again later.") is shown and loading stops (form is *not* left stuck on the spinner).
...
- **R2.2** If the selected employee has **one or more** associated projects (`project.employeeId === employeeId`), the *latest* one (most recent `startDate`; if `startDate` is missing/tied, order is otherwise unspecified) is auto-filled — Project, Customer, and Bill Rate all populate without further user action.
- **R2.3** When multiple projects match, the Project dropdown remains scoped to that same matching set (see R4.1), so the user can switch to any other project belonging to this employee — switching re-fills Customer/Bill Rate from the newly picked project (see R4.2), it does not clear them.
- **R2.4** If the selected employee has **zero** associated projects, any previously selected Customer, Project, and Bill Rate are cleared back to blank/0 (they are not left showing a stale, possibly-invalid combination).
...
- **R3.1** The Customer dropdown only lists customers associated with the currently selected employee's projects. If no employee is selected, all customers are listed.
- **R3.2** Selecting a Customer sets `customerId`/`customerName` on the invoice.
- **R3.3** If the selected Customer, scoped to the currently selected Employee (if any), has **one or more** matching projects, the *latest* one (most recent `startDate`) is auto-filled — including its own Employee/Bill Rate. If an Employee was already selected, it is preserved/reconfirmed as part of this fill.
...
- **R3.5** If the Customer has **zero** matching projects (given the current Employee scope, if any), no project is auto-selected and the current Project/Bill Rate are left as-is.
...
- **R4.2** Selecting a Project auto-fills **all** of: Employee, Customer, Project name, and Bill Rate — since a project record already carries its own `employeeId`/`employeeName`/`customerId`/`customerName`/`billRate`. This is true regardless of what was previously selected for Employee/Customer (the project's own linkage wins).
...
- **R5.1** At all times, after any of Employee/Customer/Project changes, the three fields should represent a *mutually consistent* combination — i.e. it should never be possible to submit an invoice where the selected Customer or Project doesn't actually belong to the selected Employee, when that data is known.
- **R5.2** When a selection (Employee or Customer) has multiple matching projects, the system auto-picks the latest by `startDate` (R2.2, R3.3) rather than leaving fields blank — ambiguity is resolved with a sensible default, not by forcing the user to pick first. The user remains free to switch to any other matching project via the Project dropdown (R2.3, R3.4).
...
- **R9.1** Invoice Term is populated from the selected Project's own `invoiceTerm` field whenever Employee/Customer/Project selection resolves a project (R2.2, R3.3, R4.2 — via `applyProjectSelection`), normalized from the backend's string code to a `Number` so it matches the dropdown's option values. It is also directly editable (re-selectable) by the user afterward.
...
- **R6.1** Submit is blocked (error modal: "Please fill in all mandatory fields before submitting.") if any of the following are missing: Customer, Invoice Id, Bill Rate, Invoice Month, Hours. (Note: Employee and Project are *not* checked here even though their `<Form.Item>`s are marked `required` — see Known Gaps.)
...
- **R7.1** Clicking **Clear** resets every field to its initial blank state: Employee, Customer, Project (both the underlying data in `generalDetails` and the visible dropdown selections), Bill Rate, Hours, Invoice Id, Invoice Month, Start/End Date, and Total.
...
- **R8.1** Employee, Customer, and Project dropdowns support type-to-search filtering on their visible label text (case-insensitive substring match).
...
- **R8.4** Bill Rate can also be edited manually via its input field after being auto-filled, and manual edits are not overwritten unless Employee/Customer/Project selection changes again.
...
- **Customer lockout for zero-project employees (found while writing tests, high severity).** `customersForEmployee` is derived strictly from `projectsForEmployee` (R3.1): `customers.filter(v => projectsForEmployee.some(p => p.customerId === v.customerId))`. When the selected employee has **zero** projects, `projectsForEmployee` is `[]`, so `customersForEmployee` is *also* `[]` — the Customer dropdown renders "No data" with nothing selectable, for **any** customer, not just ones tied to other employees. Since Customer is a required field (R6.1) and there is no `allowClear`/free-text escape hatch, **an invoice can never be submitted for an employee with zero projects** — Submit will always fail the required-field check. Covered by the "finding" tests in `NewInvoice.test.jsx` (R3 and Known gaps sections). Confirm with product whether employees are expected to always have at least one project, or whether the Customer dropdown should fall back to the full customer list when the employee-scoped list is empty.
- Employee and Project `<Form.Item>` are marked `required`, but `handleSubmit`'s manual validation (R6.1) does not check `employeeId` or `projectId` — an invoice can currently be submitted with no Employee/Project selected as long as Customer/Invoice Id/Bill Rate/Invoice Month/Hours are filled. Confirm whether this is intentional before writing a test that asserts submission is blocked without an Employee/Project.
- `handleEmployeeChange` reads `selectedEmployee?.Name` (capitalized) from the `employees` list, while the Employee dropdown itself renders `employee.name` (lowercase); confirmed live that the `/employees/getEmployees` API only returns lowercase `name`, so this line always sets `employeeName` to `undefined`. In practice this is **effectively unobservable today**: for any employee with 1+ projects it's immediately overwritten by `applyProjectSelection` (R2.2, which uses the project's own correctly-cased `employeeName`); for an employee with 0 projects, the Customer lockout above blocks submission entirely before the bad value could ever reach the backend. Still worth fixing for correctness/future-proofing (e.g. if the Customer lockout above gets fixed, this bug becomes live again), but no current UI path can prove it via an end-to-end/black-box test — it would need a unit-level test of `handleEmployeeChange` in isolation, or a code-level assertion.
